<template>
  <div>
     ...
     <div style="float:right;" class="container">
      ...
      <div class="inner">
        ...
        <bk-popconfirm trigger="click" :ext-cls="'asadsadsads'" width="288" @confirm="addData()">
          <div slot="content">
              <bk-compose-form-item>
                <h3>课程名: <bk-input v-model='create_name' type='text'/></h3>
                <h3>课程简介: <bk-input v-model='create_content' type='text'/></h3>
                <h3>教师工号ID: <bk-input v-model='create_teach_id' type='text'/></h3>
              </bk-compose-form-item>
          </div>
    ...
    <bk-table style="margin-top: 15px;"
      ...
      <bk-table-column label="课程名称" prop="name"></bk-table-column>
          ...
          <bk-popconfirm trigger="click" :ext-cls="'asadsadsads'" width="280" @confirm="changeData(props.row.id)">
              <div slot="content">
                  <bk-compose-form-item>
                    ...
                    <h3>课程简介: <bk-input v-model="update_content" type='text'/></h3>
                    <h3>教师工号: <bk-input v-model="update_teach_id" type='text'/></h3>
                  </bk-compose-form-item>
              </div>
...
export default {
  ...
  methods: {
    getData () {
      axios.get(host + '/courses/', { // 获取data列表中的数据
        headers: {
          'Authorization': 'Bearer ' + this.token
        },
        responseType: 'json'
      }).then(response => {
        console.log('课程信息' + response.data)
    ...
    addData () {
      if (this.create_name && this.create_content && this.create_teach_id) {
        axios.post(host + '/courses/', JSON.parse(JSON.stringify(
          {
    ...
    changeData (id) {
      if (this.update_name && this.update_content && this.update_teach_id) {
        axios.put(host + '/courses/' + id + '/', JSON.parse(JSON.stringify(
          {
            'id': id,
    ...
    update_init(name, content, teach_id) {
    ...
    search_data () { // 通过v-model绑定的textcontent获取文本内容，通过下滑框对应的value1获取下拉框选择的对象  搜索数据
      if (!this.textcontent) {
        ...
        axios.get(host + '/courses/search/', { // 获取data列表中的数据
          headers: {
            'Authorization': 'Bearer ' + this.token
          },
          responseType: 'json',
    ...
    to_excel (data) { // 转化成excel
      let str = '<tr><td>课程ID</td><td>课程名</td><td>课程简介</td><td>教师ID</td><td>教师名称</td><td>上课学生</td></tr>'
