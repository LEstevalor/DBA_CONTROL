<template>
  <div>
     <bk-compose-form-item class="select-demo">
       <div>&nbsp;</div>
        <bk-select v-model="value1" style="width: 140px" size="large" :clearable="false">
          <bk-option id="ip" name="专业名称" @click="option_college"></bk-option>
          <bk-option id="source" name="学院名称" @click="option_dean"></bk-option>
          <bk-option id="content" name="简介信息" @click="option_infor"></bk-option>
        </bk-select>
        <bk-input style="width: 400px" size="large" v-model=textcontent v-if="value1 === 'ip'" key:1 placeholder="请输入专业名称" :left-icon="'bk-icon icon-search'"></bk-input>
        ...
        <bk-popconfirm trigger="click" :ext-cls="'asadsadsads'" width="288" @confirm="addData()">
          <div slot="content">
              <bk-compose-form-item>
                ...
                <h3>专业人数: <bk-input v-model='create_count' type='text' disabled="true"/></h3>
                <h3>专业简介: <bk-input v-model='create_content' type='text'/></h3>
              </bk-compose-form-item>
          </div>
    ...
    <bk-table style="margin-top: 15px;"
      ...
      <bk-table-column label="专业名称" prop="ip"></bk-table-column>
      <bk-table-column label="学院名称" prop="source"></bk-table-column>
      <bk-table-column label="专业人数" prop="count"></bk-table-column>
      ...
      <bk-table-column label="操作" width="150">
        <template slot-scope="props">
          <bk-popconfirm trigger="click" :ext-cls="'asadsadsads'" width="288"
              ...
              <div slot="content">
                  <bk-compose-form-item>
                    <h3>专业名称: <bk-input v-model="props.row.ip" type='text' disabled="true"/></h3>
                    <h3>学院名称: <bk-input v-model='props.row.source' type='text' disabled="true"/></h3>
                    <h3>专业人数: <bk-input v-model='props.row.count' type='text' disabled="true"/></h3>
                    <h3>专业简介: <bk-input v-model='update_content' type='text'/></h3>
                  </bk-compose-form-item>
              </div>
...
export default {
  ...
  methods: {
    getData () {
      axios.get(host + '/major/', { // 获取data列表中的数据
        headers: {
    ...
    addData () {
      ...
      if (this.create_college_name && this.create_major_name && this.create_content) {
        axios.post(host + '/major/', JSON.parse(JSON.stringify(
          {
            'college_name': this.create_college_name,
            'name': this.create_major_name,
            'count': '0',
    ...
    changeData (id, ip) {
      // 修改数据 （当创建对应专业的班级人数时，专业人数才会改动）
      if (this.update_content) {
        axios.put(host + '/major/' + id + '/', JSON.parse(JSON.stringify(
          {
            'name': ip,
    ...
    search_data () { // 通过v-model绑定的textcontent获取文本内容，通过下滑框对应的value1获取下拉框选择的对象  搜索数据
      if (!this.textcontent) {
        ...
        axios.get(host + '/major/search/', { // 获取data列表中的数据
          headers: {
            'Authorization': 'Bearer ' + this.token
          },
          responseType: 'json',
          ...
          console.log('筛选的专业信息：' + response.data)
          this.data = response.data.major // 列表的数据和data是绑一起的
          this.pagination['count'] = this.data.length
    ...
    to_excel (data) { // 转化成excel
      let str = '<tr><td>专业名称</td><td>学院名称</td><td>专业人数</td><td>简介</td><td>专业ID</td></tr>' // 列标题
